...
my_ontology = Ontology('src/gen/ontology.json')
...
def trim_silence(s):
    data, sr = audiofile.read(s)
    # data, sr = librosa.load(s, sr=44100)
    # sr, data = read(s)
    # data, sr = sf.read(s)
    if len(data.shape) > 1:
    ...
    for index, sample_data in tqdm(dataset.iterrows(), total=dataset.shape[0]):
        ...
        if dry_run:
            ...
            if not os.path.exists(d):
                ...
                print("File exists, skipping symlink")
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--datasets_dir', type=str, default='/scr/BinauralCuratedDataset',
        help="Path to directory containing all datasets.")
    parser.add_argument(
        '--class_definitions', type=str, default='src/gen/Classes.yaml',
        help="Path to class susbet selection.")
    parser.add_argument(
        '--ontology', type=str, default='src/gen/ontology.json',
        help="Path to ontology definition.")
    parser.add_argument(
        '--fg_output_dir', type=str, default='/scr/BinauralCuratedDataset/scaper_fmt',
        help="Path to directory to write scaper formatted data.")
    parser.add_argument(
        '--bg_output_dir', type=str, default='/scr/BinauralCuratedDataset/bg_scaper_fmt',
        help="Path to directory to write scaper formatted data.")
    parser.add_argument(
        '--dry_run', action='store_true', help="Dry run. Do not write any files.")
    args = parser.parse_args()

    datasets = ['Misophonia', 'FSD50K', 'ESC-50']
    # datasets = ['Misophonia']
    dataset_types = ['train', 'val', 'test']

    for i, dset in enumerate(datasets):
        print(f'Collecting dataset {dset}...')
        collector_name = 'src/gen/compile/' + dset.replace('-', '').lower() + '_label_collector.py'
        os.system(f'python {collector_name} --dataset_dir {os.path.join(args.datasets_dir, datasets[i]) }')
