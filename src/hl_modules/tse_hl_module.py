...
class InformedTFGridNet(nn.Module):
    def __init__(self, tsh, emb):
        super(InformedTFGridNet, self).__init__()
        self.tsh = tsh
        self.emb = emb

#     def forward(self, mixture, enrollment):
#         embedding = self.emb(enrollment)
        
#         input_dict = {'mixture':mixture, 'embedding': embedding}
#         outputs = self.tsh(input_dict)

#         return outputs
    
    def forward(self, mixture, embedding):        
        input_dict = {'mixture':mixture, 'embedding': embedding}
        outputs = self.tsh(input_dict)
...
class PLModule(object):
    def __init__(self, sr, 
        ...
        self.tsh_mdl = utils.import_attr(tsh_model)(**tsh_model_params)
        self.enrollment_mdl = utils.import_attr(enrollment_model)(**enrollment_model_params)
        self.model = InformedTFGridNet(self.tsh_mdl, self.enrollment_mdl)
    ...
    def _step(self, batch, batch_idx, step='train'): 
        ...
        # Forward pass
        enrollment = inputs['enrollment'].clone()
        mix = inputs['mixture'].clone()
        embedding = targets['embedding_gt'].clone()
        
        # outputs = self.model(mix, enrollment)
        outputs = self.model(mix, embedding)
