...
class PLModule(object):
    def __init__(self, sr, 
        ...
        self.loss_fn = torch.nn.CosineEmbeddingLoss(margin=0)
    ...
    def _step(self, batch, batch_idx, step='train'):
        ...
        # Forward pass
        clean_enrollment = targets['enrollment_clean']
        mix = inputs['mixture']
        
        outputs = self.model(dict(enrollment=mix))
        
        est = outputs['output']
        gt = targets['embedding']
        
        # Compute loss
        loss = self.loss_fn(est.squeeze(1), gt.squeeze(1), torch.ones(est.shape[0], device=est.device))
        ...
        with torch.no_grad():
            ...
            if (f'stat/{step}_input_snr' not in self.statistics) or (not self.statistics[f'stat/{step}_input_snr']['logged']):
                for i in range(batch_size):
                    snr_val = self.snr_metric(est=mix[i].unsqueeze(0), gt=clean_enrollment[i].unsqueeze(0), mix=mix[i].unsqueeze(0))
        ...
        sample = {
            'mixture':mix,
            'target':clean_enrollment,
            'n_tgt_speakers':targets['num_target_speakers']
        }
