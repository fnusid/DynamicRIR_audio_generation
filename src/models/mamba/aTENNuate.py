...
class Denoiser(nn.Module):
    def __init__(self, 
        ...
        # )
        self.last_ssms = nn.Sequential(
            self.ssm_block(1, True), self.ssm_block(1, False), 
        )
...
if __name__ == '__main__':
    ...
    # breakpoint()
    fixed_target = 3
    target_audio = audio[fixed_target , :].clone()[:16000*6] #gt
    audio[fixed_target,:] *= 0
    audio = audio.unsqueeze(0)[:,:, :16000*6]
    
    sa = Denoiser().to(device)
