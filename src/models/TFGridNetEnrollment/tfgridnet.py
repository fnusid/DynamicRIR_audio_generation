...
class TFGridNet(nn.Module):
    ...
    def __init__(
        ...
        self.deconv = nn.ConvTranspose2d(emb_dim, spk_emb_dim, ks, padding=( self.t_ksize - 1, 1))
        self.freq_pool = nn.Linear(self.n_freqs, 1)

    def forward(self, input_stft: torch.Tensor) -> torch.Tensor:
        """
        ...
        for ii in range(self.n_layers):            
            batch = self.blocks[ii](batch) # [B, T, Q, C]

        batch = batch.permute(0, 3, 1, 2) # [B, C, T, Q]
        
        batch = self.deconv(batch)  # [B, D, T, F]
        batch = self.freq_pool(batch).squeeze(-1) # [B, D, T]
        embedding = torch.mean(batch, axis=-1).unsqueeze(1) # [B, 1, D]
